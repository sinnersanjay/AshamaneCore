...
void System::cpuid(CPUIDFunction func, uint32& eax, uint32& ebx, uint32& ecx, uint32& edx) {
#if defined(__i386__) || defined(__x86_64__)
    // x86/x86_64: use cpuid instruction via register-pinned variables
    uint32 a = static_cast<uint32>(func), b = 0, c = 0, d = 0;
    asm volatile(
        "cpuid"
        : "=a"(a), "=b"(b), "=c"(c), "=d"(d)
        : "a"(a), "c"(c)
...
#else
    // Non-x86 architecture: cpuid is not available, return zeroes
    eax = ebx = ecx = edx = 0;
#endif
}
