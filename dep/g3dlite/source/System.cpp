...
// SIMD include (used only on x86/x86_64)
...
static bool checkForCPUID() {
    // CPUID is only available on x86/x86_64. On other architectures
    // (such as ARM), report that CPUID is not available so that the
    // rest of the code can safely skip CPUID-dependent paths.
...
void System::cpuid(CPUIDFunction func, uint32& eax, uint32& ebx, uint32& ecx, uint32& edx) {
    // Use volatile asm with a memory clobber and explicit register variables
    // to avoid impossible constraint errors on both 32-bit and 64-bit GCC.
    register uint32 a asm("eax") = static_cast<uint32>(func);
    register uint32 b asm("ebx") = 0;
    register uint32 c asm("ecx") = 0;
    register uint32 d asm("edx") = 0;
    asm volatile("cpuid"
        : "+r"(a), "+r"(b), "+r"(c), "+r"(d)
        :
        : "memory");
